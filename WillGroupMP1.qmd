...
The three potential hypotheses that I will propose all relate to potential correlations between different variables. Firstly "Does sleep affect academic performance?". This question aims to answer whether or not there is any correlation between the amount of hours you sleep and your academic success. Secondly "Does Diet/Nutrition affect academic performance?". Similar to the first question this question aims to answer whether or not eating habits affect academic success. Lastly "What is the link between mental health and stress and academic performance?". This question aims to discover whether or not mental health and stress is correlated with grades either positively or negatively.
...
numeric_data <- select_if(survey_data, is.numeric)

# Get descriptive statistics
describe(numeric_data)
```
The most interesting result from viewing the data is how certain metrics were collected. Since the forum was somewhat vague on how your supposed to answer some questions it can result in some inaccuracies. For example the question regarding height did not specify the form in which the height should be entered resulting in many different outliers. For example one student inputted "511" in order to mean 5'11 which is wildly different than how other people answered the question.
...
aggr(survey_data, col = c("navyblue", "red"), numbers = TRUE, sortVars = TRUE,
     labels = names(survey_data), cex.axis = 0.7, gap = 3,
     ylab = c("Missing Data", "Pattern"))

# Identify rows with any missing values
survey_missing <- survey_data[!complete.cases(survey_data), ]

# Save rows with missing data
write.csv(survey_missing, "SurveyMissing.csv", row.names = FALSE)

# Remove rows with any missing values
survey_data_complete <- na.omit(survey_data)
```
B and D: By observing the data it seems like the data is likely MNAR (Missing not at random) since the variables that have missing values are alcohol, gpa, and health. These categories all have more than one missing value each which is an indication that the nature of the value being missing has to do with the category itself. In addition these categories make sense to be missing as some people do not feel comfortable sharing this information even if they are anonymous. 

C: Given that there were only 10 missing values and 85 total observations I decided that it would be simplest to just remove the observations with missing values. I decided to still save the missing values to a dataset called SurveyMissing.
