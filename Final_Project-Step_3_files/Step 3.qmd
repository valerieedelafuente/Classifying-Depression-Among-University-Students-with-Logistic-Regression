...
depression_data <- depression_data %>% filter(sleep_duration != "Other")
depression_data$sleep_duration <- as.numeric(factor(
  depression_data$sleep_duration, levels = c("Less than 5 hours", "5-6 hours", "7-8 hours", "More than 8 hours")))
...
The column names in the original dataset were changed to the following: `id`, `gender`, `age`, `city`, `profession`, `academic_pressure`, `work_pressure`, `cum_gpa`, `study_satisfaction`, `job_satisfaction`, `sleep_duration`, `dietary_habits`, `degree`, `suicidal_thoughts`, `work_study_hours`, `financial_stress`, `fam_mental_illness`, and `depression`.
...
<<<<<<< HEAD
-   `sleep_duration`: invalid entries were properly handled.
=======
...
=======
There were 3 missing observations present in the dataset, coming from the `financial_stress` variable. This accounts for less than 0.1% of the dataset, so we can easily remove them. With the data processed and tidied, we can move onto modeling.
...
>>>>>>> parent of 3bdfffd (changs)
### Hypothesis 1: Students with moderate to healthy dietary habits have lower rates of depression compared to students with unhealthy dietary habits.
...
```
=======
### Hypothesis 3: Students with higher collective reported stressors will have higher rates of depression than students with lower collective reported stressors.
>>>>>>> parent of 3bdfffd (changs)
