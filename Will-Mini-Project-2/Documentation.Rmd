...
author: "William Nelson, Matthew Arteaga, Hayden Galleta"
date: "`r Sys.Date()`"
output: pdf_document
---
...
Our group did not face many challenges in particular when developing our app to investigate the relationship between the variables of the `mtcars` dataset. One thing that required a bit of trial and error was deciding the ranges for the slider features that we added to both the bar and scatter plot graphs. We had to go through a few iterations to decide which range would be most practical to the user. Through team collaboration we were able to work out which additional features would prove to be useful to the user in their own exploration of the data set and the relationship between variables. Our main takeaway was that for the `mtcars` data set in particular, the scatter plot graphic was the most useful graphic overall in terms of investigating relationships between variables, and working towards building a model to predict key variables, such as mpg.