...
# Since we're interested in Student Depression, 
# we'll removing observations that are not Student.
depression_data <- depression_data %>% filter(profession == "Student")
...
```{r}
library(corrplot)
cor(depression_data %>% select_if(is.numeric) %>% select(-c(job_satisfaction,work_pressure))) %>% corrplot(type="lower", diag = FALSE, method = "square")
```
...
Students have a mean age of about 26 years old, with the majority of students being between 18 and 30 years old.

```{r}
prop.table(table(depression_data$depression))
```

We see that close to 60% of students responded they experience Depression.

```{r}
prop.table(table(depression_data$dietary_habits))
```

```{r}
prop.table(table(depression_data$sleep_duration))
```

```{r}
prop.table(table(depression_data$academic_pressure))
```

```{r}
prop.table(table(depression_data$financial_stress))
```
...
maybe not include or include and talk about reason for all 0's

```{r}
depression_data %>% 
  ggplot(aes(x=work_pressure, fill = depression)) + 
  geom_bar(position = "dodge") + 
  labs(x = "Work Pressure 0-5 (0 = No Pressure)", y = "Number of Students",
       title = "Depresion by Work Pressure",
       fill = "Depression")
```
